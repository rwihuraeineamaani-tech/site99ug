...
export default function CalendarItemDialog({ open, onOpenChange, onSaved, defaultDate, editing, duplicate = false }: { open: boolean; onOpenChange: (v: boolean) => void; onSaved: () => void; defaultDate: string; editing: CalendarItem | null; duplicate?: boolean }) {
  ...
  const [reminder, setReminder] = useState("15"); const [work, setWork] = useState(""); const [workOptions, setWorkOptions] = useState<WorkOption[]>([]); const [busy, setBusy] = useState(false);
  ...
  useEffect(() => {
    ...
    setInterval(item?.interval_n ?? 1); setByweekday(item?.byweekday ?? []); setUntil(item?.until ?? ""); setOccurrences(item?.occurrences ? String(item.occurrences) : ""); setReminder(item?.reminder_minutes == null ? "" : String(item.reminder_minutes)); setWork(item?.work_kind ? `${item.work_kind}|${item.work_id ?? ""}|${item.work_label ?? ""}|${item.work_path ?? ""}` : "");
