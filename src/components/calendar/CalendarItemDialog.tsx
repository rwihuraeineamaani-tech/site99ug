...
export default function CalendarItemDialog({ open, onOpenChange, onSaved, defaultDate, editing, duplicate = false }: { open: boolean; onOpenChange: (v: boolean) => void; onSaved: () => void; defaultDate: string; editing: CalendarItem | null; duplicate?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[100dvh] max-w-2xl overflow-y-auto rounded-none sm:max-h-[90vh] sm:rounded-lg"><DialogHeader><DialogTitle>{duplicate ? "Duplicate calendar item" : editing ? "Edit calendar item" : "New calendar item"}</DialogTitle></DialogHeader><div className="space-y-5 pb-[env(safe-area-inset-bottom)]">
    ...
    <div><label className="eyebrow text-ink-faint">Visibility</label><div className="mt-1.5 grid gap-2 sm:grid-cols-2">{([{ v: "private", icon: Lock, t: "Private", n: "Team sees only Busy and the time." }, { v: "team", icon: Users, t: "Team", n: "Team can see all event details." }] as const).map((o) => <button key={o.v} type="button" onClick={() => setVisibility(o.v)} className={`min-h-11 rounded-lg border p-3 text-left ${visibility === o.v ? "border-signal bg-signal/[0.07]" : "border-rule"}`}><o.icon className="mb-2 h-4 w-4 text-signal" /><div className="text-sm font-semibold">{o.t}</div><div className="mt-1 text-[11px] text-ink-soft">{o.n}</div></button>)}</div></div>
    <div><label className="eyebrow text-ink-faint">Scheduling</label><div className="mt-1.5 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => setStrictness("warn")} className={`min-h-11 rounded-lg border p-3 text-left ${strictness === "warn" ? "border-signal bg-signal/[0.07]" : "border-rule"}`}><div className="text-sm font-semibold">Warn only</div><div className="text-[11px] text-ink-soft">Show the clash but allow booking.</div></button><button type="button" onClick={() => setStrictness("hard")} className={`min-h-11 rounded-lg border p-3 text-left ${strictness === "hard" ? "border-signal bg-signal/[0.07]" : "border-rule"}`}><div className="text-sm font-semibold">Do not schedule</div><div className="text-[11px] text-ink-soft">Require a leadership override.</div></button></div></div>
    ...
    <div className="sticky bottom-0 -mx-6 flex flex-wrap gap-2 border-t border-rule bg-paper px-6 py-3"><Button onClick={() => save()} disabled={busy}>{editing && !duplicate ? "Save changes" : "Add to calendar"}</Button>{editing && !duplicate && <Button variant="outline" onClick={() => save(true)} disabled={busy} className="gap-1.5"><Copy className="h-4 w-4" />Duplicate</Button>}{editing && !duplicate && <Button variant="destructive" onClick={remove} disabled={busy} className="sm:ml-auto gap-1.5"><Trash2 className="h-4 w-4" />Delete</Button>}</div>
