...
export default function SystemAdmin() {
  ...
  const [newDelegation, setNewDelegation] = useState({ delegator_id: "", delegate_id: "", department: "", starts_at: "", ends_at: "" });
  ...
  const createWorkflow = async () => {
    const name = window.prompt("Workflow name"); if (!name?.trim()) return;
    const department = window.prompt("Department", "Operations"); if (!department?.trim()) return;
    const key = `${department}_${name}`.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "");
    const { data, error } = await supabase.from("approval_workflows").insert({ workflow_key: key, name: name.trim(), department: department.trim(), entity_type: key, description: null }).select("id").single();
    if (error) return toast.error(error.message); toast.success("Workflow created"); await load(); if (data?.id) setSelectedId(data.id);
  };
  const addDelegation = async () => {
    if (!newDelegation.delegator_id || !newDelegation.delegate_id || !newDelegation.starts_at || !newDelegation.ends_at) return toast.error("Complete the cover period and both people.");
    if (newDelegation.delegator_id === newDelegation.delegate_id) return toast.error("Choose a different backup person.");
    const { error } = await supabase.from("approval_delegations").insert({ ...newDelegation, department: newDelegation.department || null, starts_at: new Date(newDelegation.starts_at).toISOString(), ends_at: new Date(newDelegation.ends_at).toISOString() });
    if (error) return toast.error(error.message); toast.success("Approval cover scheduled"); setNewDelegation({ delegator_id: "", delegate_id: "", department: "", starts_at: "", ends_at: "" }); await load();
  };
  ...
  return <AdminShell eyebrow="System administration" title="Control centre." active={tab} nav={[
    ...
    {!loading && tab === "workflows" && <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_300px]"><div className="surface overflow-hidden rounded-lg h-[620px]"><ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={(_, n) => setSelectedNode(n.id)} fitView proOptions={{ hideAttribution: true }}><Background gap={22} color="hsl(var(--rule))" /><MiniMap className="!bg-[hsl(var(--paper-sunken))] !border !border-rule" nodeColor={(n) => nodeTone(((n.data as { kind?: WorkflowNodeKind }).kind ?? "approval"))} /><Controls /></ReactFlow></div><aside className="border-l border-rule pl-4 space-y-5"><div><div className="eyebrow text-ink-faint mb-3">Add step</div><div className="grid grid-cols-2 gap-2">{WORKFLOW_KINDS.map((k) => <Button key={k.key} size="sm" variant="outline" onClick={() => addNode(k.key)}>{k.label}</Button>)}</div></div>{currentNode ? <div className="space-y-3"><div className="eyebrow text-ink-faint">Selected step</div><input className={field} value={String((currentNode.data as { label?: string }).label ?? "")} onChange={(e) => patchNode({ label: e.target.value })} /><select className={field} value={String((currentNode.data as { config?: { role?: string } }).config?.role ?? "")} onChange={(e) => patchConfig({ role: e.target.value || undefined })}><option value="">Approver role…</option>{TEAM_ROLES.map((r) => <option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select><select className={field} value={String((currentNode.data as { config?: { user_id?: string } }).config?.user_id ?? "")} onChange={(e) => patchConfig({ user_id: e.target.value || undefined })}><option value="">Or named person…</option>{team.map((m) => <option key={m.user_id} value={m.user_id}>{nameOf(m.user_id)}</option>)}</select><label className="text-xs text-ink-soft">Deadline hours<input className={`${field} mt-1`} type="number" min="1" value={Number((currentNode.data as { config?: { sla_hours?: number } }).config?.sla_hours ?? 24)} onChange={(e) => patchConfig({ sla_hours: Number(e.target.value) })} /></label>{(currentNode.data as { kind?: string }).kind === "condition" && <div className="space-y-2"><input className={field} placeholder="Field, e.g. amount" value={String((currentNode.data as { config?: { field?: string } }).config?.field ?? "")} onChange={(e) => patchConfig({ field: e.target.value })} /><select className={field} value={String((currentNode.data as { config?: { operator?: string } }).config?.operator ?? "equals")} onChange={(e) => patchConfig({ operator: e.target.value })}>{["equals","not_equals","greater_than","less_than","contains"].map((x) => <option key={x}>{x.replace(/_/g, " ")}</option>)}</select><input className={field} placeholder="Comparison value" value={String((currentNode.data as { config?: { value?: string } }).config?.value ?? "")} onChange={(e) => patchConfig({ value: e.target.value })} /></div>}<label className="flex gap-2 text-xs"><input type="checkbox" checked={Boolean((currentNode.data as { config?: { exclude_requester?: boolean } }).config?.exclude_requester)} onChange={(e) => patchConfig({ exclude_requester: e.target.checked })} /> Requester cannot approve</label><label className="flex gap-2 text-xs"><input type="checkbox" checked={Boolean((currentNode.data as { config?: { distinct_actor?: boolean } }).config?.distinct_actor)} onChange={(e) => patchConfig({ distinct_actor: e.target.checked })} /> Require a different approver</label><Button variant="destructive" size="sm" onClick={() => { setNodes((x) => x.filter((n) => n.id !== currentNode.id)); setEdges((x) => x.filter((e) => e.source !== currentNode.id && e.target !== currentNode.id)); setSelectedNode(null); }}>Remove step</Button></div> : <p className="text-xs text-ink-soft">Select a step to set its owner and deadline.</p>}</aside></div>
