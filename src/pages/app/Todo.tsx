...
const FILTERS: { value: "all" | "overdue" | "today" | "next" | TodoKind; label: string }[] = [
  ...
  { value: "content", label: "Content" }, { value: "shoots", label: "Shoots" }, { value: "approvals", label: "Approvals" }, { value: "strategy", label: "Strategy" }, { value: "finance", label: "Finance" }, { value: "operations", label: "Operations" },
];
...
export default function TodoPage() {
  ...
  return <AppShell eyebrow="To-Do"><Seo title="To-Do — Site 99" description="Every action assigned to you across Site 99." path="/app/todo" noindex /><PageHeader eyebrow="To-Do" title="Your move." lede={loading ? "Loading your work…" : `${items.length} action${items.length === 1 ? "" : "s"} across content, shoots, approvals, strategy, finance and operations.`} actions={<div className="flex gap-2"><Segmented value={view} onChange={setMode} options={[{ value: "cards", label: "Cards" }, { value: "list", label: "List" }]} /><Button variant="outline" size="sm" onClick={reload}>Refresh</Button></div>} />
